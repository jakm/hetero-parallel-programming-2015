// Excercise 2.2 - Tiled Matrix-Matrix Multiplication
...
__global__ void matrixMultiplyShared(float *A, float *B, float *C, int numARows,
  ...
  for (int offset = 0; offset < (numAColumns-1)/TILE_WIDTH+1; offset++) { // i < numBRows
    ds_A[ty][tx] = (row < numARows && offset*TILE_WIDTH+tx < numAColumns
                    ? A[row*numAColumns + offset*TILE_WIDTH+tx]
                    : 0.0);
    ds_B[ty][tx] = (offset*TILE_WIDTH+ty < numBRows && col < numBColumns
                    ? B[(offset*TILE_WIDTH+ty)*numBColumns + col]
                    : 0.0);
